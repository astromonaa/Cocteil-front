...
const {form, hasProducts, createOrder} = useOrders()
...
<template>
  <div :class="isMobile? 'mobile-wrapper' : 'wrapper'">
    ...
    <form :class="{'mobile-form': isMobile}">
      <InputText v-model="form.name" id="name" placeholder="ФИО" label="ФИО"/>
      <InputText v-model="form.tel" id="tel" type="tel" placeholder="Контактный телефон" label="Контактный телефон"/>
      <InputText v-model="form.postIndex" id="index" placeholder="Индекс" label="Индекс"/>
      <InputText v-model="form.address" id="address" placeholder="Адрес доставки" label="Адрес доставки"/>
      <InputText v-model="form.email" id="email" type="email" placeholder="Электронная почта"
                 label="Электронная почта"/>
      <TextArea v-model="form.comment" id="comment" placeholder="Комментарий" label="Комментарий"
                :class="{'mt-22': isMobile}"/>
    ...
    <div class="warning">
      ...
      <NuxtLink to="/deliveryterms" :class="{'font-12': isMobile}">Сроки доставки указаны при условии наличия товара.
        Заказ, сделанный со вт по чт отправляется в пн. Заказ, сделанный с пт по вс отправляется в пт
      </NuxtLink>
    </div>
    ...
    <CircleButtonBig
      ...
      :is-pink="false"
      :style="{margin: isMobile ? '50px auto 20px' : '0 auto'}"
      centered
    />
  </div>
</template>
