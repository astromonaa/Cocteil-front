...
<template>
  <div :class="['wrapper', {'mobile-wrapper': isMobile}]">
    ...
    <div
        v-for="faq in faq" :key="faq.title"
        :class="[{open: faq.open}, 'detail']"
        @click="openFaq(faq)"
    >
      <div class="detail-top">
        <span>{{ faq.title }}</span>
      </div>
      <div class="detail-bottom">{{ faq.text }}</div>
    </div>
  </div>
</template>
...
.detail-top {
  ...
  &:after {
    ...
    height: 20px;
    position: absolute;
    background: url(/img/svg/20x20/plus.svg);
    right: 0;
...
.mobile-wrapper {
  ...
  .detail:last-child > .detail-bottom {
    margin-top: 40px;
  }
